...
/*************************************************************/
{
    ...
    if( (FEAttr( sym ) & FE_DLLIMPORT )
     && ( FEAuxInfo( sym, CALL_BYTES ) == NULL ) ){
        leaf = TGLeaf( BGName( CG_FE, sym, TypeAddress( CGTY_POINTER ) ) );
#ifndef NDEBUG
        EchoAPI( "CGFEName( %s, %t ) declspec(dllimport)", sym, tipe );
        hdlAdd( CG_NAMES, leaf );
#endif
        leaf = CGUnary( O_POINTS, leaf, CGTY_POINTER );
    } else {
...
/*****************************************************************/
{
    ...
    retn = TGLeaf( BGTempName( temp, TypeAddress( CGTY_POINTER ) ) );
    hdlAdd( CG_NAMES, retn );
    return EchoAPICgnameReturn( retn );
#else
    tipe = tipe;
    return( TGLeaf( BGTempName( temp, TypeAddress( CGTY_POINTER ) ) ) );
#endif
}
...
/**********************************************************************************************/
{
    expr = TGen( expr, TypeAddress( CGTY_DEFAULT ) );
...
/*******************************************/
{
    ...
    hdlUseOnce( CG_NAMES, name );
    retn = TGTmpLeaf( TGen( name, TypeAddress( CGTY_DEFAULT ) ) );
    hdlAddUnary( CG_NAMES, retn, name );
    return EchoAPICgnameReturn( retn );
#else
    return( TGTmpLeaf( TGen( name, TypeAddress( CGTY_DEFAULT ) ) ) );
#endif
}
...
/*********************************************/
{
    ...
    hdlAllUsed( CG_NAMES );
#endif
    BGTrash( TGen( TGTrash( name ), TypeAddress( CGTY_DEFAULT ) ) );
}
...
/**************************************************/
{
    ...
    addr = TGen( name, TypeAddress( CGTY_DEFAULT ) );
