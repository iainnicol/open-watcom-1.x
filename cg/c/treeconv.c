...
/*********************************************************************/
    ...
    if( TypeClass( frum ) <= I4 ) {
        ...
        case TN_BINARY: /* go left, right*/
            ...
            case O_PLUS:
            case O_MINUS:
                if( name->tipe->refno == CGTY_HUGE_POINTER ) break;
                if( name->u.left->tipe->refno == CGTY_HUGE_POINTER ) break;
                if( name->rite->tipe->refno == CGTY_HUGE_POINTER ) break;
                can_demote = DemoteTree( name->u.left, tipe, just_test );
                if( can_demote ) {
...
/***************************************************************/
    ...
    if( name->class == TN_CONS ) {
        if( name->tipe->refno == CGTY_DEFAULT ) {
