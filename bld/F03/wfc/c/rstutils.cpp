...
sym_id  STAdd( char *name, int length )
{
    ...
    sym = (sym_id)FMemAlloc( sizeof( symbol ) + AllocName( length ) );
...
// common block).
    //TODO: fix the whol freeLink/freechain thing.
    return( (sym_id)FreeLink( (void**)sym ) );
}
...
// Free statement function header.
    ...
    if( sym->ns.si.sf.header != NULL ) {
        FreeChain( (void**)&sym->ns.si.sf.header->parm_list );
