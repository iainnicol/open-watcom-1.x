...
void            FiniAuxInfo(void)
{
    ...
    FreeChain((void**)&DefaultLibs );
    // in case of fatal error, FiniAuxInfo() is called
    // from TDPurge()
#if ( _CPU == 8086 || _CPU == 386 )
    FreeChain((void**) &ArrayInfo );
#endif
    FreeChain((void**) &DependencyInfo );
    AsmSymFini();
}
...
// Process aux information for an array.
    ...
    new_arr = static_cast<arr_info*>(FMemAlloc( sizeof( arr_info ) + arr_len ));
    new_arr->link = NULL;
...
void    AddDependencyInfo( source_t *fi )
{
    ...
    if( p != NULL ) {
        ...
        if( fstat( ((a_file *)(fi->fileptr))->handle, &stat_info ) != -1 ) {
            new_dep = static_cast<dep_info*>(FMemAlloc( sizeof( dep_info ) + strlen( p ) ));
            new_dep->link = NULL;
            strcpy( new_dep->fn, p );
...
static  void    AddDefaultLib( char *lib_ptr, int lib_len, char priority )
{
    ...
    new_lib = static_cast<default_lib*>(FMemAlloc( sizeof( default_lib ) + lib_len ));
    new_lib->link   = NULL;
...
//========================================================
    ...
    aux = (aux_info *)FMemAlloc( sizeof( aux_info ) + name_len );
...
static  void    DupCallBytes( aux_info *dst, aux_info *src )
{
    ...
    new_seq = static_cast<byte_seq*>(FMemAlloc( sizeof( byte_seq ) + seq_len ));
...
static  void    DupParmInfo( aux_info *dst, aux_info *src )
{
    ...
    ++size;
    new_reg_set = static_cast<hw_reg_set*>(FMemAlloc( size * sizeof( hw_reg_set ) ));
...
static  void    DupObjectName( aux_info *dst, aux_info *src )
{
    ...
    new_name = static_cast<char*>(FMemAlloc( strlen( src->objname ) + sizeof( char ) ));
...
static  void    DupArgInfo( aux_info *dst, aux_info *src )
{
    ...
    while( args != NULL ) {
        new_arg = static_cast<pass_by*>(FMemAlloc( sizeof( pass_by ) ));
...
static  void            ObjectName(void)
{
    ...
    name = static_cast<char*>(FMemAlloc( obj_len + sizeof( char ) ));
...
//===============================================================
static  void    InsertFixups( char *buff, unsigned i )
{
                        // additional slop in buffer to simplify the code
    char                temp[MAXIMUM_BYTESEQ+2*sizeof(byte)];
    struct asmfixup     *fix;
    ...
    seq = static_cast<byte_seq*>(FMemAlloc( sizeof( byte_seq ) + i ));
    ...
    for(;;) {
        if( *TokStart == '"' ) {
            ...
            *(char *)(TokEnd - sizeof( char )) = NULLCHAR;
            ...
            AsmCodeBuffer  = (unsigned char *)buff;
            AsmLine( &TokStart[1] );
...
//==================================
    ...
    regs = (hw_reg_set *)FMemAlloc( ( num_sets + 1 ) * sizeof( hw_reg_set ) );
...
static  void    GetArgList(void)
{
    ...
    for(;;) {
        ...
        arg = (pass_by*)FMemAlloc( sizeof( pass_by ) );
        arg->link = NULL;
        arg->info = pass_info;
        *curr_arg = arg;
        curr_arg = &arg->link;
