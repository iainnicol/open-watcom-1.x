...
static  void    ConstBase( uint base )
{
    ...
    {
        CITNode->value.intstar1 = static_cast<intstar1>(strtoul(CITNode->opnd, &end, base));
        CITNode->size = sizeof( intstar1 );
        CITNode->typ  = TY_INTEGER_1;
    }else if( CITNode->opnd_size <= 5 )
    {
        CITNode->value.intstar2 = static_cast<intstar2>(strtoul(CITNode->opnd, &end, base));
