...
//======================================
    ...
    unsigned_16 classType;
    unsigned_16 flags;

    flags = sym->ns.flags;
    classType = flags & (unsigned_16)SY_CLASS;
    if( classType == SY_PARAMETER ) return( NAME_PARAMETER );
    if( classType == SY_COMMON ) return( NAME_COMMON );
    if( classType == SY_SUBPROGRAM ) {
...
//=================================================
    ...
    if( ClassMsg[idx].className != NULL ) return( ClassMsg[idx].className );
...
//==============================================
    ...
    char        buff[MAX_SYMLEN+1];
    char        className[MAX_MSGLEN+1];

    STGetName( sym, buff );
    Error( errcod, GetClass( SymClass( sym ), className ), buff );
}
...
void    PrevDef( sym_id sym )
{
    ...
    STGetName( sym, buff );
    Error( VA_PREV_DEF_NAM, buff, GetClass( SymClass( sym ), className ) );
}
...
void    KnownClassErr( int errcod, uint idx )
{
    ...
    Error( errcod, GetClass( idx, className ) );
}
...
void    ClassErr( int errcod, sym_id sym )
{
    ...
    Error( errcod, GetClass( SymClass( sym ), className ) );
}
...
void    IllName( sym_id sym )
{
    ...
    STGetName( sym, buff );
    Error( VA_ILL_USE, GetClass( SymClass( sym ), className ), buff,
                       StmtName( stmt ) );
}
