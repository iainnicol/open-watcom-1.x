...
static boolean dupCompare( TYPE test1, TYPE test2 )
{
    ...
    case TYP_GENERIC:
        ...
        if( test1->u.g.name != 0 && test2->u.g.name != 0 && test1->u.g.name != test2->u.g.name ) {
            return( FALSE );
        }
        break;
    case TYP_ERROR:
...
static void saveType( void *e, carve_walk_base *d )
{
    ...
    CLASSINFO *save_info;
    TYPE save_type;
    char *save_get, *save_put, *save_gname;
    void *save_base;
    ...
    case TYP_MEMBER_POINTER:
        ...
        break;
    case TYP_GENERIC:
        save_gname  = s->u.g.name;
        s->u.g.name = NameGetIndex( s->u.g.name );
        break;
    case TYP_PROPERTY:
        save_get   = s->u.p.get;    
        save_put   = s->u.p.put;
...
static void readTypes( type_pch_walk *type_data )
{
    ...
    for(;;) {
        ...
        case TYP_MEMBER_POINTER:
            ...
            break;
        case TYP_GENERIC:
            t->u.g.name = NameMapIndex( pch->u.g.name );
            break;
        case TYP_PROPERTY:
