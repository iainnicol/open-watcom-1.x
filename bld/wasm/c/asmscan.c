   ...
   for( ;; )
   {
      switch( tolower( *ptr ) )
      {
         ...
         case 'a':
            ...
            break;
         case 'b':
            if( base == 0 && OK_NUM( BINARY ) && !isxdigit( ptr[1] ) &&
                tolower( ptr[1] ) != 'h' )
            {
               base = 2;
               extra = 1;
         ...
         case 'c':
            ...
            break;
         case 'd':
            if( base == 0 && OK_NUM( DECIMAL ) && !isxdigit( ptr[1] ) &&
                tolower( ptr[1] ) != 'h' )
            {
...
*/
{
   ...
   static char    stringbuf[MAX_LINE_LEN];   /* buffer to store strings in */
   ...
   for( ;; )
   {
      ...
      if( isalpha( *ptr ) || *ptr == '_' || *ptr == '$' || *ptr == '@' ||
          *ptr == '?' || *ptr == '\\' || ( *ptr == '.' && buf_index == 0 ) )
      {
