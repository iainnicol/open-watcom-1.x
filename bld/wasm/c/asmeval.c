...
{
   if( tok_1->type == type && tok_2->type == type )
   {
       return( TRUE );
   }
   else
...
/* Perform the operation between token_1 and token_2 */
{
   ...
   switch( AsmBuffer[index]->token )
   {
      ...
      case T_COLON:
         ...
         else if( token_2->type == EXPR_ADDR && token_1->type == EXPR_ADDR &&
                  token_1->override == EMPTY && token_1->instr == EMPTY &&
                  token_1->value == 0 && token_1->base_reg == EMPTY &&
      ...
      case T_INSTR:
         ...
         break;
      case T_UNARY_OPERATOR:
         if( ( AsmBuffer[index]->value == T_OFFSET ) &&
             ( token_2->type == EXPR_CONST ) )
         {
         }
...
static int evaluate( expr_list *operand1, int *i, int end,
                     enum process_flag proc_flag, bool (*is_expr)(int) )
/******************************/
{
   expr_list   operand2;
   char        token_needed;
   ...
   do
   {
      ...
      /*:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::*/
      while( ( *i <= end ) && ( op_sq_bracket_level > op_sq_bracket ) &&
   ...
   } while ( ( next_operator == TRUE ) || ( ( proc_flag == PROC_BRACKET ) &&
             !cmp_token( *i, T_CL_BRACKET ) &&
             !cmp_token( *i, T_CL_SQ_BRACKET ) && ( *i < end ) ) );
   if( op_sq_bracket_level != op_sq_bracket )
   {
      if( error_msg )
         AsmError( BRACKETS_NOT_BALANCED );
   ...
   tokens */
{
   ...
   for( start = 0; start < TokCnt; start++ )
   {
      if( AsmBuffer[start]->token == T_NOOP )
      {
         for( end = start + 1;; end++ )
         {
            if( AsmBuffer[end]->token != T_NOOP || end == TokCnt )
            {
               break;
            }
         }
...
/*************************************************************************/
{
   ...
   while( i < TokCnt && i <= end_tok )
   {
      if( is_expr1( i ) )
      {
         ...
         if( num == 1 && AsmBuffer[start]->token == T_REG &&
         ...
         else
         {
            ...
            error_msg = flag_msg;
            if( evaluate( &result, &i, i + num, PROC_BRACKET,
                          is_expr1 ) == ERROR )
            {
               return( ERROR );
            }
