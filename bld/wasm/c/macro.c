...
extern  int_8  DefineProc; // TRUE if the definition of procedure has not ended
...
/****************************/
{
  ...
  if( Options.ideal )
   ...
   for( ; ; )
   {
      ...
      while( isspace( *ptr ) )
      ptr++;
      if( lineis( ptr, "for" ) || lineis( ptr, "forc" ) ||
          lineis( ptr, "irp" ) || lineis( ptr, "irpc" ) ||
          lineis( ptr, "rept" )|| lineis( ptr, "repeat" ) )
      {
         nesting_depth++;
      }
      ...
      while( isspace( *ptr ) )
         ptr++;
      if( lineis( ptr, "macro" ) || lineis( ptr, "for" ) ||
          lineis( ptr, "forc" )  || lineis( ptr, "irp" ) ||
          lineis( ptr, "irpc" )  || lineis( ptr, "rept" )||
          lineis( ptr, "repeat" ) )
      {
         nesting_depth++;
      }
...
/*****************************/
{
   ...
   if( AsmBuffer[count]->token == T_FINAL )
      return( tok_count );
         ...
         else
         {
            ...
            for( ; ; )
            {
               ...
               else
               {
                  ...
                  {
                     if( AsmBuffer[count+1]->token == T_FINAL )
                        count++;
