   ...
   switch( state )
   {
      ...
      case SYM_PROC:
         ...
         if( ( Code->mem_type == MT_EMPTY || Code->mem_type == MT_SHORT ||
               Code->mem_type == MT_NEAR ) && sym->mem_type != MT_WORD &&
               sym->mem_type != MT_DWORD && sym->mem_type != MT_FWORD &&
         ...
         if( Code->mem_type == MT_EMPTY && sym->mem_type != MT_EMPTY )
         {
            switch( sym->mem_type )
            {
               ...
               case MT_PROC:
                  ...
                  if( IS_JMPCALLN( Code->info.token ) && ( Code->mem_type == MT_FAR ) )
                  {
                     Code->info.token++;
                  }
                  break;
