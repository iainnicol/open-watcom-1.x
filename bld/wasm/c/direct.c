...
typedef struct
{
   asm_sym  *symbol;          /* segment or group that is to be associated
                                 with the register */
...
/***********************************/
/* wipes out preceding and tailing spaces, and make sure token contains only
   one word */
{
   char  *ptrhead;
   char  *ptrend;
...
// but PTR operator should be resolved globally
   /* Fix up PTR size */
   switch( type )
   {
      case MOD_COMPACT:
...
/******************/
{
   ...
   else
   {
      ...
      /**/myassert( n >= 0 );
      sym = AsmGetSymbol( AsmBuffer[n]->string_ptr );
...
static int MakeComm( char *name, memtype mem_type, bool already_defd,
                     int number, memtype distance )
/********************/
{
   dir_node       *dir;
   struct asm_sym *sym;
