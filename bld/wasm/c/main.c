...
global_options Options =
{
   ...
   TRUE,                /* Watcom C name mangler */
   TRUE,                /* stdcall at number */
   TRUE,                /* mangle stdcall */
   FALSE,               /* write listing */
   FALSE                /* tasm ideal mode syntax */
};
...
/*********************************/
{
   ...
   for( s = id; *s != '\0'; s++ )
   {
      ...
      if( !( lwr_char == '_' || lwr_char == '.' || lwr_char == '$' ||
             lwr_char == '@' || lwr_char == '?' || isdigit( lwr_char ) ||
          islower( lwr_char ) ) )
      {
         return( ERROR );
      }
...
static void Set_FL( void )
{
   get_fname( GetAFileName(), LST ); Options.write_listing = TRUE;
}
...
{
   Options.watcom_c_mangler = FALSE;
}

static void Set_ZLD( void )
{
