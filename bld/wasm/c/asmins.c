...
uint_8                        CheckSeg;   /* if checking of opened segment is
                                             needed */
...
struct asm_tok                *AsmBuffer[MAX_TOKEN];  /* buffer to store
                                                         tokens */
...
/******************************************/
{
   ...
   {
      if( strnicmp( string, &AsmChars[ AsmOpcode[ i ].index ],
                    AsmOpcode[ i ].len ) == 0 &&
...
#if defined( _STANDALONE_ )
   if( ( opndx->sym->state == SYM_SEG ) || ( opndx->sym->state == SYM_GRP ) ||
       ( opndx->instr == T_SEG ) )
   {
      sym32 = 0;
   }
   ...
   switch( Code->info.token )
   {
      ...
      case T_MOV:
         ...
         else if( ( op1 == OP_M ) || ( op2 == OP_M ) )
         {
            ...
            {
               temp = ( temp & BIT_67 ) | ( ( temp & BIT_012 ) << 3 ) |
                      ( ( temp & BIT_345 ) >> 3 );
               if( addr_32( Code ) && ( temp == D32 ) || !addr_32( Code ) && ( temp == D16 ) )
               {
