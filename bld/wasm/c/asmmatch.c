...
*/
{
   ...
   else if( ins->allowed_prefix == FWAIT )
   {
      AsmCodeByte( OP_WAIT );
            ...
            ( !rCode->use32 ) && ( ins->allowed_prefix != NO_FWAIT ) &&
            ( ( ins->allowed_prefix == FWAIT ) ||
            ( ( ins->cpu&P_FPU_MASK ) != P_NO87 ) ) )
   {
      AsmCodeByte( OP_WAIT );
   ...
   if( ins->opnd_dir )
   {
      ...
      rCode->info.rm_byte = ( tmp & 0xc0 ) | ( ( tmp >> 3 ) & 0x7 ) |
                            ( ( tmp << 3 ) & 0x38 );
   }
   switch( ins->rm_info )
   {
   ...
   while( AsmOpTable[i].token == Code->info.token )
   {
      ...
      switch( asm_op1 )
      {
         ...
         case OP_I_3:      // for INT only
            ...
            break;
         case OP_NONE:
            if( cur_opnd == OP_NONE && Code->info.opnd_type[OPND2] == OP_NONE )
            {
               return( output( i ) );
            }
            break;
         default:
             ...
             */
            if( ( asm_op1 & cur_opnd ) && ( ( ( cur_opnd & OP_M_ANY ) == 0 ) ||
                ( ( asm_op1 & OP_M_DFT ) == ( cur_opnd & OP_M_DFT ) ) ) )
            {
          ...
          AsmOpTable[*i].token == instruction )
   {
      ...
      switch( asm_op2 )
      {
         ...
         case OP_I:
            ...
            break;
         case OP_I8_U:
            if( ( cur_opnd != OP_I8 ) && ( cur_opnd != OP_I8_U ) &&
                ( cur_opnd != OP_I16 ) )
            {
               break;
            }
         ...
         case OP_I8:
            ...
            else if( ( cur_opnd & OP_I ) && ( InsFixups[OPND2] == NULL ) &&
                     ( ( last_opnd & OP_R16 ) || ( last_opnd & OP_M16 ) &&
                     ( MEM_TYPE( Code->mem_type, WORD ) ) ) )
            {
               ...
               else
               {
                  break;
                     ...
                     ( ( last_opnd & OP_R32 ) || ( last_opnd & OP_M32 ) &&
         ...
         case OP_I_1:
            ...
            break;
         case OP_M16:
            if( cur_opnd & OP_M && ( MEM_TYPE( Code->mem_type, WORD ) ||
         ...
         case OP_M:
            ...
            break;
         default:
            if( ( asm_op2 & ( OP_MMX | OP_XMM ) ) && ( cur_opnd & asm_op2 ) )
            {
               if( check_3rd_operand( *i ) == ERROR )
                  break;
               if( output( *i ) == ERROR )
