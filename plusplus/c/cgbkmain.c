    ...
    target_offset_t offset )    // - offset of object just CTOR'ed/DTOR'ed
{
    ...
    if( ! IbpReference( NULL, &sym, &ref, &offset_ref ) ) {
        expr = CgExprPop();
        if( offset > 0 ) {
           expr = CGBinary( O_MINUS, expr, CgOffset( offset ), CGTY_POINTER );
        }
        expr = CGLVAssign( CgSymbol( sym ), expr, CGTY_POINTER );
        CgExprPush( expr, CGTY_POINTER );
    }
}
        ...
        switch( se->base.se_type ) {
          ...
          case DTC_TEST_FLAG :
            if( flags & DGRP_DIRECT ) {
                ...
                expr = CgFetchType( expr, CGTY_UINT_1 );
                expr = CGBinary( O_AND, expr, CgOffset( cond.mask ), CGTY_UINT_1 );
                CgControl( O_IF_TRUE, expr, CGTY_UINT_1, lab );
    ...
    boolean branch_on )         // - TRUE ==> branch if on
{
    ...
    if( fctl->has_cdtor_val ) {
        ...
        expr = CGBinary( O_AND, expr, CGInteger( mask, CGTY_UINT_1 ), CGTY_UINT_1 );
        expr = CGCompare( O_NE, expr, CGInteger( 0, CGTY_UINT_1 ), CGTY_BOOL );
    ...
    TYPE class_type )           // - class type of ctor/dtor
{
    ...
    */
    ...
    /* unsigned const *cdp; */
    cdp = CGTemp( CGTY_POINTER );
    /* unsigned cdv; */
    cdv = CGTemp( CGTY_UNSIGNED );
    /* cdp = this->vbptr; */
    info = class_type->u.c.info;
    vb_offset = info->vb_offset;
    e1 = CGFEName( (cg_sym_handle)fctl->this_sym, CGTY_POINTER );
    e1 = CGUnary( O_POINTS, e1, CGTY_POINTER );
    e1 = CGBinary( O_PLUS, e1, CgOffset( vb_offset ), CGTY_POINTER );
    e1 = CGUnary( O_POINTS, e1, CGTY_POINTER );
    e2 = CGTempName( cdp, CGTY_POINTER );
    e1 = CGAssign( e2, e1, CGTY_POINTER );
    CGDone( e1 );
    RingIterBeg( info->bases, base ) {
        if( _IsVirtualBase( base ) ) {
            if( TypeCtorDispRequired( class_type, base->type ) ) {
                /* cdv = cdp[i] + offsetof(scope,vbptr); */
                e1 = CGTempName( cdp, CGTY_POINTER );
                e1 = CGUnary( O_POINTS, e1, CGTY_POINTER );
                e2 = CgOffset( base->vb_index * TARGET_UINT );
                e1 = CGBinary( O_PLUS, e1, e2, CGTY_POINTER );
                e1 = CGUnary( O_POINTS, e1, CGTY_UNSIGNED );
                e1 = CGBinary( O_PLUS, e1, CgOffset( vb_offset ), CGTY_POINTER );
                e2 = CGTempName( cdv, CGTY_UNSIGNED );
                e1 = CGAssign( e2, e1, CGTY_UNSIGNED );
                CGDone( e1 );
                /* *(this + (cdv - sizeof(unsigned))) = cdv; */
                e1 = CGTempName( cdv, CGTY_UNSIGNED );
                e1 = CGUnary( O_POINTS, e1, CGTY_UNSIGNED );
                e2 = CgOffset( TARGET_UINT );
                e2 = CGBinary( O_MINUS, e1, e2, CGTY_UNSIGNED );
                e1 = CGFEName( (cg_sym_handle)fctl->this_sym, CGTY_POINTER );
                e1 = CGUnary( O_POINTS, e1, CGTY_POINTER );
                e2 = CGBinary( O_PLUS, e1, e2, CGTY_POINTER );
                e1 = CGTempName( cdv, CGTY_UNSIGNED );
                e1 = CGUnary( O_POINTS, e1, CGTY_UNSIGNED );
                e1 = CGAssign( e2, e1, CGTY_UNSIGNED );
                CGDone( e1 );
    ...
    boolean vbptr )             // - a vbptr?
{
    ...
    e1 = CGFEName( (cg_sym_handle)fctl->this_sym, CGTY_POINTER );
    e1 = CGUnary( O_POINTS, e1, CGTY_POINTER );
    e1 = CGBinary( O_PLUS, e1, CgOffset( delta ), CGTY_POINTER );
    e2 = CGFEName( (cg_sym_handle)table_sym, CGTY_POINTER );
    if( ! vbptr ) {
        e2 = CGBinary( O_PLUS, e2, CgOffset( CgDataPtrSize() ), CGTY_POINTER );
    }
    e1 = CGAssign( e1, e2, CGTY_POINTER );
    CGDone( e1 );
}
    ...
    boolean vbptr )             // - a vbptr?
{
    ...
    */
    ...
    e1 = CGBinary( O_PLUS, e1, e2, CGTY_POINTER );
    e1 = CGUnary( O_POINTS, e1, CGTY_UNSIGNED );
    e2 = CGFEName( (cg_sym_handle)fctl->this_sym, CGTY_POINTER );
    e2 = CGUnary( O_POINTS, e2, CGTY_POINTER );
    e1 = CGBinary( O_PLUS, e2, e1, CGTY_POINTER );
    e1 = CGBinary( O_PLUS, e1, CgOffset( vb_offset + delta ), CGTY_POINTER );
    e2 = CGFEName( (cg_sym_handle)table_sym, CGTY_POINTER );
    if( ! vbptr ) {
        e2 = CGBinary( O_PLUS, e2, CgOffset( CgDataPtrSize() ), CGTY_POINTER );
    }
    ...
    target_offset_t delta )     // - delta amount
{
    ...
    */
    ...
    e1 = CGFEName( (cg_sym_handle)fctl->this_sym, CGTY_POINTER );
    e2 = CgOffset( delta );
    e1 = CGPreGets( op, e1, e2, CGTY_POINTER );
    CGDone( e1 );
}
    ...
    FN_CTL* fctl )              // - function information
{
    ...
    */
    ...
    e1 = CGUnary( O_POINTS, e1, CGTY_POINTER );
    e2 = CgOffset( TARGET_UINT );
    e1 = CGBinary( O_MINUS, e1, e2, CGTY_POINTER );
    e2 = CGUnary( O_POINTS, e1, CGTY_UNSIGNED );
    e1 = CGFEName( (cg_sym_handle)fctl->this_sym, CGTY_POINTER );
    e1 = CGPreGets( O_MINUS, e1, e2, CGTY_POINTER );
    CGDone( e1 );
}
    ...
    target_offset_t index )     // - virtual base index
{
    ...
    */
    ...
    e2 = CGUnary( O_POINTS, e1, CGTY_UNSIGNED );
    e1 = CGFEName( (cg_sym_handle)fctl->this_sym, CGTY_POINTER );
    e1 = CGPreGets( O_PLUS, e1, e2, CGTY_POINTER );
    CGDone( e1 );
}
...
//
            ...
            switch( ins_value.uvalue ) {
              ...
              case CO_LE :
                CgExprPush( CGCompare( cg_opcode, op1, op2, exprn_type )
                          , CGTY_BOOL );
                exprn_type = CGTY_BOOL;
                break;

              case CO_AND_AND :
              case CO_OR_OR :
                CgExprPush( CGFlow( cg_opcode, op1, op2 ), CGTY_BOOL );
                exprn_type = CGTY_BOOL;
                break;
              ...
              case CO_VASTART :
              { cg_name x;
                x = CGVarargsBasePtr( CGTY_POINTER );
                op1 = CGLVAssign( op1, x, CGTY_POINTER );
                x = CGInteger( TARGET_POINTER, CGTY_INTEGER );
                x = CGBinary( O_PLUS, op1, x, CGTY_POINTER );
                CgExprPush( CGAssign( x, op2, CGTY_INTEGER ), CGTY_INTEGER );
              } break;
#endif
          ...
          { cg_type op_type;                // - type of operand
            ...
            switch( ins_value.uvalue ) {
              case CO_EXCLAMATION :
                CgExprPush( CGFlow( cg_opcode, op1, NULL ), CGTY_BOOL );
                exprn_type = CGTY_BOOL;
                break;
              case CO_UPLUS :
              ...
              case CO_FAR16_TO_POINTER:
                CgExprPush( CGUnary( O_PTR_TO_NATIVE, op1, CGTY_POINTER )
                          , CGTY_POINTER );
                break;
              case CO_POINTER_TO_FAR16:
                CgExprPush( CGUnary( O_PTR_TO_FOREIGN, op1, CGTY_POINTER )
                          , CGTY_POINTER );
                break;
          ...
          { cg_name op1;                    // - object copied to
            ...
            CgExprPush( op1, CGTY_POINTER );
            exprn_type = CGTY_POINTER;
          } break;
          ...
          { cg_name ref;                    // - reference operand
            ...
            if( FunctionDeclarationType( type ) == NULL ) {
                far_type = CGTY_LONG_POINTER;
            } else {
                far_type = CGTY_LONG_CODE_PTR;
            }
            ...
            ref = CgOffsetExpr( ref
                              ...
                              , CGTY_POINTER );
            ref = CgFetchType( ref, CGTY_UINT_2 );
            CgExprPush( ref, CGTY_UINT_2 );
          } break;
          ...
          { cg_name expr;                   // - top expression
            expr = CgExprPop();
            expr = CGUnary( O_STACK_ALLOC, expr, CGTY_POINTER );
            CgExprPush( expr, CGTY_POINTER );
          } break;
#endif
...
//
          ...
          { SYMBOL sym;                     // - function symbol
            ...
            if( fctl->debug_info
             && ( GenSwitches & DBG_LOCALS ) ) {
                DBModSym( (cg_sym_handle)sym, CGTY_DEFAULT );
            }
            BlkPosnPush( NULL );
          ...
          { SYMBOL retn_sym;
            ...
            if( retn_sym == NULL ) {
                if( SpecialFunction( fctl->func ) == SPFN_MAIN ) {
                    ...
                    cgname = CGInteger( 0, CGTY_INTEGER );
                    cgtype = CGTY_INTEGER;
                } else {
                    cgname = NULL;
          ...
          case IC_DTOR_STATIC :     // STATIC SYMBOL NEEDS DTOR
            ...
            CgCommaOptional( CgDtorStatic( ins_value.pvalue ), CGTY_POINTER );
            break;
...

        {
          ...
          { cg_name top_expr;       // - top expression
            ...
            if( ! fctl->temp_dtoring ) {
                ...
                if( NULL != sv_expr ) {
                    top_expr = CgComma( top_expr, sv_expr, CGTY_POINTER );
                    top_type = CGTY_POINTER;
                }
            }
            CgExprPush( top_expr, top_type );
          } break;
...
//
...

        {
          ...
          { RT_DEF def;                     // - defn for R/T call
            ...
            CgExprPush( CgRtCallExec( &def ), CGTY_INTEGER );
          } break;
        ...
        { SE* try_se;                       // try SE for catch
          ...
          case IC_THROW_RO_BLK :            // SET THROW R/O BLOCK
            sig_thunk_genned = TRUE;
            CgExprPush( ThrowRo( ins_value.pvalue ), CGTY_DEFAULT );
            break;
          ...
          case IC_SET_LABEL_SV :            // SET STATE AT LABEL
            CgDone( FstabEmitStateVar( BlkPosnCurr(), fctl ), CGTY_POINTER );
            break;
            ...
            if( init->defn != NULL ) {
                if( NULL != dtor_last_reqd ) {
                    ...
                    expr = CgCallBackCtorStart( NULL
                                              , CGTY_POINTER
                                              , se );
                    CgCommaBefore( expr, CGTY_POINTER );
            ...
            if( init->defn != NULL
             && init->obj_se != NULL ) {
                expr = CgCallBackCtorDone( NULL
                                         , CGTY_POINTER
                                         , init->obj_se );
                CgCommaOptional( expr, CGTY_POINTER );
            }
          } break;
