    ...
    FN_CTL* fctl )              // - function information
{
    ...
    if( set_flag ) {
        op_mask = CGInteger( cond.mask, CGTY_UINT_1 );
        op_flg = CGLVPreGets( O_OR, op_flg, op_mask, CGTY_UINT_1 );
    } else {
        op_mask = CGInteger( 0xFF - cond.mask, CGTY_UINT_1 );
        op_flg = CGLVPreGets( O_AND, op_flg, op_mask, CGTY_UINT_1 );
    }
    return op_flg;
}
    ...
    boolean set_flag )          // - TRUE ==> set the flag; FALSE ==> clear
{
    ...
    patch = BEPatch();
    op_mask = CGPatchNode( patch, CGTY_UINT_1 );
    if( set_flag ) {
    ...
    op_flg = CgSymbolPlusOffset( cond.sym, cond.offset );
    op_flg = CGLVPreGets( opcode, op_flg, op_mask, CGTY_UINT_1 );
    CgExprPush( op_flg, CGTY_POINTER );
}
