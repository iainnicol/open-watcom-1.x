...
// Process end of a complex expression.
    ...
    CGDone( CGBinary( O_COMMA, XPop(), XPop(), CGTY_DEFAULT ) );
}
...
// Process PAUSE/STOP statement.
    ...
    lit = GetPtr();
    if( lit == NULL ) {
        arg = CGInteger( 0, CGTY_LOCAL_POINTER );
    } else {
        arg = CGBackName( ConstBack( lit ), CGTY_LOCAL_POINTER );
    }
    CGAddParm( handle, arg, CGTY_LOCAL_POINTER );
