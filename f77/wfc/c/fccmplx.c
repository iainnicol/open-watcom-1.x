...
//============================================
    ...
    if( typ == CGTY_COMPLEX ) return( CGTY_SINGLE );
    if( typ == CGTY_DCOMPLEX ) return( CGTY_DOUBLE );
    return( CGTY_LONGDOUBLE );
}
...
// Pass a complex value to a subprogram.
    ...
    if( typ == CGTY_SINGLE ) {
        c_type = CGTY_COMPLEX;
    } else if( typ == CGTY_DOUBLE ) {
        c_type = CGTY_DCOMPLEX;
    } else {
        c_type = CGTY_XCOMPLEX;
    }
    tmp = AllocTmp( c_type );
