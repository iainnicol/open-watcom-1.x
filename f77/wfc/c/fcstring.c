...
// Get pointer from SCB.
    ...
    return( CGUnary( O_POINTS, SCBPtrAddr( scb ), CGTY_GLOBAL_POINTER ) );
}
...
// Get pointer to length in SCB.
    ...
    return( StructRef( scb, BETypeLength( CGTY_GLOBAL_POINTER ) ) );
}
...
// Get pointer to flags in SCB.
    ...
    return( StructRef( scb, BETypeLength( CGTY_CHAR ) ) );
}
...
// Get length from SCB.
    ...
    return( CGUnary( O_POINTS, SCBLenAddr( scb ), CGTY_UNSIGNED ) );
}
...
// Do concatenation operation.
    ...
    count = num_args;
    while( count > 0 ) {
        CGAddParm( call, StkElement( count ), CGTY_LOCAL_POINTER );
        --count;
    }
    PopStkElements( num_args );
    CloneCGName( dest, &dest_1, &dest_2 );
    CGAddParm( call, dest_1, CGTY_LOCAL_POINTER );
    if( num_args != 1 ) {
        CGAddParm( call, CGInteger( num_args, CGTY_UNSIGNED ), CGTY_UNSIGNED );
    }
    return( CGBinary( O_COMMA, CGCall( call ), dest_2, CGTY_LOCAL_POINTER ) );
}
...
// Perform N character assignment of non optimal lengths.
    ...
    if( OZOpts & OZOPT_O_SPACE || !equal ) {
        CGAddParm( call, CGInteger( src_len, CGTY_INTEGER ), CGTY_INTEGER );
    } else {
        // Special but common case, so we optimize it.
        CGAddParm( call, CGInteger( src_len & TAIL_MASK, CGTY_INTEGER ),
                        CGTY_INTEGER );
        CGAddParm( call, CGInteger( src_len >> TAIL_SHIFT, CGTY_INTEGER ),
                        CGTY_INTEGER );
    }

    CGAddParm( call, SCBPointer( XPop() ), CGTY_LOCAL_POINTER );
    if( !equal ) {
        CGAddParm( call, CGInteger( dst_len - src_len, CGTY_INTEGER ), CGTY_INTEGER );
    }
    CGAddParm( call, SCBPointer( dst ), CGTY_LOCAL_POINTER );
    XPush( CGBinary( O_COMMA, CGCall( call ), dst2, CGTY_LOCAL_POINTER ) );
}
...
// Get element size for character*(*) arrays.
    ...
    if( sym->ns.flags & SY_VALUE_PARM ) {
        return( CGInteger( 0, CGTY_INTEGER ) );
    } else if( Options & OPT_DESCRIPTOR ) {
        return( SCBLength( CGUnary( O_POINTS, CGFEName( sym, CGTY_POINTER ),
                                    CGTY_POINTER ) ) );
    } else {
        return( CGUnary( O_POINTS, CGFEName( FindArgShadow( sym ), CGTY_INTEGER ),
                CGTY_INTEGER ) );
    }
}
...
// Do substring operation.
    ...
    if( char_var == NULL ) { // i.e. chr(i:i)
        len = CGInteger( GetInt(), CGTY_INTEGER );
        if( Options & OPT_BOUNDS ) {
            CloneCGName( first_1, &first_1, &last );
            last = CGBinary( O_PLUS, last, len, CGTY_INTEGER );
            last = CGBinary( O_MINUS, last, CGInteger( 1, CGTY_INTEGER ),
                             CGTY_INTEGER );
        }
    } else {
        last = XPop();
        if( last == NULL ) {
            if( char_var->ns.xt.size == 0 ) {
                ...
                last = CGInteger( char_var->ns.xt.size, CGTY_INTEGER );
            }
        } else {
            XPush( last );
        ...
        if( !( Options & OPT_BOUNDS ) ) {
            ...
            len = CGBinary( O_MINUS, last, first_2, CGTY_INTEGER );
            len = CGBinary( O_PLUS, len, CGInteger( 1, CGTY_INTEGER ), CGTY_INTEGER );
        }
    }
    dest = GetPtr();
    if( Options & OPT_BOUNDS ) {
        call = InitCall( RT_SUBSTRING );
        CGAddParm( call, CGFEName( dest, CGTY_CHAR ), CGTY_LOCAL_POINTER );
        CGAddParm( call, last, CGTY_INT_4 );
        CGAddParm( call, first_1, CGTY_INT_4 );
        CGAddParm( call, src, CGTY_LOCAL_POINTER );
        XPush( CGBinary( O_COMMA, CGCall( call ), CGFEName( dest, CGTY_CHAR ),
                         CGTY_LOCAL_POINTER ) );
    } else {
        ptr = CGBinary( O_PLUS, SCBPointer( src ),
                        CGBinary( O_MINUS, first_1, CGInteger( 1, CGTY_INTEGER ),
                                  CGTY_INTEGER ),
                        CGTY_GLOBAL_POINTER );
        CGTrash( CGAssign( SCBLenAddr( CGFEName( dest, CGTY_CHAR ) ),
                           len, CGTY_INTEGER ) );
        ...
        // is not lost.
        XPush( CGLVAssign( SCBPtrAddr( CGFEName( dest, CGTY_CHAR ) ),
                           ptr, CGTY_GLOBAL_POINTER ) );
// Don't do it the following way:
//        CGTrash( CGAssign( SCBPtrAddr( CGFEName( dest, CGTY_CHAR ) ),
//                           ptr, CGTY_GLOBAL_POINTER ) );
//        XPush( CGFEName( dest, CGTY_CHAR ) );
    }
}
