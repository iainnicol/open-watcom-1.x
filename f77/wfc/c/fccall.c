...
// Return subprogram cg type.
    ...
    if( (sym->ns.flags & SY_SUBPROG_TYPE) == SY_PROGRAM ) return( CGTY_INTEGER );
    if( (sym->ns.flags & SY_SUBPROG_TYPE) == SY_SUBROUTINE ) return( CGTY_INTEGER );
    // must be a function
    if( sym->ns.typ == TY_CHAR ) return( CGTY_INTEGER );
...
// Start a subprogram.
    ...
    if( CommonEntry != NULL ) {
        ...
        CGSelect( sel, CGUnary( O_POINTS, CGFEName( EPValue, CGTY_INTEGER ),
                                CGTY_INTEGER ) );
...
//==============================
    ...
    if( Options & OPT_TRACE ) {
        ...
        CGAddParm( handle, CGBackName( TraceEntry, CGTY_POINTER ), CGTY_POINTER );
...
// Generate return value for common entry point.
    ...
    if( EntryWithAltRets() ) {
        CGReturn( CGUnary( O_POINTS, CGFEName( ReturnValue, CGTY_INTEGER ),
                           CGTY_INTEGER ), CGTY_INTEGER );
    } else {
        CGReturn( NULL, CGTY_INTEGER );
    }
}
    ...
    while( ep != NULL ) {
        ...
        if( ( ep->id->ns.flags & SY_SUBPROG_TYPE ) != SY_SUBROUTINE ) {
            ...
            if( ep->id->ns.typ == TY_CHAR ) {
                val = CGUnary( O_POINTS, val, CGTY_POINTER );
                CGAddParm( call, val, CGTY_POINTER );
                if( !(Options & OPT_DESCRIPTOR) ) {
                    val = CGFEName( FindArgShadow( ReturnValue ), CGTY_INTEGER );
                    val = CGUnary( O_POINTS, val, CGTY_INTEGER );
                    CGAddParm( call, val, CGTY_INTEGER );
                }
            } else {
                CGAddParm( call, val, CGTY_POINTER );
            }
        }
        CGAddParm( call, CGInteger( ep_count, CGTY_INTEGER ), CGTY_INTEGER );
        if( ( ep->id->ns.flags & SY_SUBPROG_TYPE ) == SY_SUBROUTINE ) {
            if( EntryWithAltRets() ) {
                ...
                if( InArgList( ep_called, arg->id ) ) {
                    ...
                    if( arg->id->ns.typ == TY_CHAR ) {
                        ...
                        if( NeedShadowArg( arg_aux ) ) {
                            cg = CGFEName( FindArgShadow( arg->id ), CGTY_INTEGER );
                            cg = CGUnary( O_POINTS, cg, CGTY_INTEGER );
                            CGAddParm( call, cg, CGTY_INTEGER );
                        }
                    } else {
                        CGAddParm( call, cg, PromoteToBaseType( arg_type ) );
                    }
                    ...
                    if( arg->id->ns.typ == TY_CHAR ) {
                        if( NeedShadowArg( arg_aux ) ) {
                            CGAddParm( call, CGInteger( 0, CGTY_INTEGER ), CGTY_INTEGER );
                        }
                    }
                }
            }
...
// Get argument pointer type.
    ...
    typ = CGType( arg );
    if( typ >= CGTY_USER_DEFINED ) return( CGTY_POINTER );
    if( BETypeLength( typ ) < BETypeLength( CGTY_POINTER ) ) return( CGTY_POINTER );
    return( typ );
}
...
//======================================
    ...
    dst = TmpPtr( AllocTmp( CGTY_CHAR16 ), CGTY_CHAR16 );
    CloneCGName( src, &src, &src_2 );
    CloneCGName( dst, &dst, &dst_2 );
    CGTrash( CGAssign( SCBLenAddr( dst_2 ), SCBLength( src_2 ), CGTY_UINT_2 ) );
    // Assumption: first field in SCB is pointer to data
    return( CGLVAssign( SCBPtrAddr( dst ),
             CGUnary( O_PTR_TO_FOREIGN, SCBPointer( src ), CGTY_GLOBAL_POINTER ),
             CGTY_GLOBAL_POINTER ) );
}
...
// Process CALL F-Code.
    ...
    if( sp->ns.flags & SY_SUB_PARM ) {
        rtn = CGUnary( O_POINTS, CGFEName( sp, CGTY_CODE_PTR ), CGTY_CODE_PTR );
    } else {
    ...
    if( sp->ns.flags & SY_INTRINSIC ) {
        if( IFVarArgs( sp->ns.si.fi.index ) ) {
            CGAddParm( call, CGInteger( num_args, CGTY_INTEGER ), CGTY_INTEGER );
        }
    } else if( (sp->ns.flags & SY_SUBPROG_TYPE) == SY_FUNCTION ) {
        if( !(Options & OPT_DESCRIPTOR) && (sp->ns.typ == TY_CHAR) ) {
            scb = GetPtr();
            arg = SCBPointer( CGFEName( scb, CGTY_CHAR ) );
#if _CPU == 386
            if( aux->cclass & FAR16_CALL ) {
                arg = CGUnary( O_PTR_TO_FOREIGN, arg, CGTY_POINTER );
            }
#endif
            CGAddParm( call, arg, CGTY_POINTER );
            CGAddParm( call, SCBLength( CGFEName( scb, CGTY_CHAR ) ), CGTY_INTEGER );
        }
    }
    idx = 0;
    while( num_args != 0 ) {
        ...
        if( (arg_code == PC_PROCEDURE) || (arg_code == PC_FN_OR_SUB) ) {
            ...
            if( (aux->cclass & FAR16_CALL) &&
                ...
                if( arg_aux->info & ARG_FAR ) {
                    cg_typ = CGTY_LONG_CODE_PTR;
                    arg = CGUnary( O_CONVERT, arg, CGTY_LONG_CODE_PTR );
                    chk_foreign = FALSE;
                }
            }
#endif
            ...
            if( arg_aux != NULL ) {
                if( arg_aux->info & ARG_FAR ) {
                    cg_typ = CGTY_LONG_POINTER;
                    arg = CGUnary( O_CONVERT, arg, CGTY_LONG_POINTER );
                }
            }
            ...
            chk_foreign = FALSE;
            if( arg_type == PT_CPLX_8 ) {
                XPopCmplx( &z, CGTY_COMPLEX );
                CGAddParm( call, z.realpart, CGTY_SINGLE );
                arg = z.imagpart;
                cg_typ = CGTY_SINGLE;
            } else if( arg_type == PT_CPLX_16 ) {
                XPopCmplx( &z, CGTY_DCOMPLEX );
                CGAddParm( call, z.realpart, CGTY_DOUBLE );
                arg = z.imagpart;
                cg_typ = CGTY_DOUBLE;
            } else if( arg_type == PT_CPLX_32 ) {
                XPopCmplx( &z, CGTY_XCOMPLEX );
                CGAddParm( call, z.realpart, CGTY_LONGDOUBLE );
                arg = z.imagpart;
                cg_typ = CGTY_LONGDOUBLE;
                ...
                if( cg_typ != new_typ ) {
                    ...
                    if( ( cg_typ == CGTY_INT_1 ) || ( cg_typ == CGTY_INT_2 ) ) {
                        cg_typ = CGTY_INT_4;
                        arg = CGUnary( O_CONVERT, arg, CGTY_INT_4 );
                    }
                }
    ...
    if( arg_vec != NULL ) {
        ...
        for( idx = 0; arg_vec[idx] != NULL; ++idx ) {
            CGAddParm( call, SCBLength( arg_vec[idx] ), CGTY_INTEGER );
        }
        FMemFree( arg_vec );
    }
    if( (sp->ns.flags & SY_SUBPROG_TYPE) == SY_SUBROUTINE ) {
        ...
        if( sp->ns.typ == TY_CHAR ) {
            if( (Options & OPT_DESCRIPTOR) || ( sp->ns.flags & SY_INTRINSIC) ) {
                scb = GetPtr();
                arg = CGFEName( scb, CGTY_CHAR );
#if _CPU == 386
                if( aux->cclass & FAR16_CALL ) {
                    arg = MkSCB16( arg );
                    arg = CGUnary( O_PTR_TO_FOREIGN, arg, CGTY_GLOBAL_POINTER );
                }
#endif
                CGAddParm( call, arg, CGTY_POINTER );
            }
            XPush( CGBinary( O_COMMA, CGCall( call ), CGFEName( scb, CGTY_CHAR ),
                             CGTY_LOCAL_POINTER ) );
        } else {
            if( (sp_type == CGTY_COMPLEX) || (sp_type == CGTY_DCOMPLEX) ||
                        (sp_type == CGTY_XCOMPLEX) ) {
                SplitCmplx( CGCall( call ), sp_type );
            } else if( sp_type >= CGTY_USER_DEFINED ) { // structure
                XPush( CGCall( call ) );
            } else {
                XPush( CGUnary( O_POINTS, CGCall( call ), sp_type ) );
            }
...
// and SCBRequired() is TRUE.
    ...
    */
        return( CGTempName( sym->ns.si.va.vi.alt_scb, CGTY_CHAR ) );
    /*
    } else {
        return( CGBackName( sym->ns.si.va.vi.alt_scb, CGTY_CHAR ) );
    }
    */
}
...
// the /nodescriptor option is specified
    ...
    if( Options & OPT_AUTOMATIC ) {
    */
        return( CGTempName( sym->ns.si.sp.alt_scb, CGTY_CHAR ) );
    /*
    } else {
        return( CGBackName( sym->ns.si.sp.alt_scb, CGTY_CHAR ) );
    }
    */
}
        ...
        if( ( sym->ns.flags & SY_SUBPROG_TYPE ) == SY_FUNCTION ) {
            if( sym->ns.typ == TY_CHAR ) {
                arg = CGUnary( O_POINTS,
                               CGFEName( ReturnValue, CGTY_POINTER ), CGTY_POINTER );
                if( Options & OPT_DESCRIPTOR ) {
                    arg = SCBLength( arg );
                } else {
                    if( CommonEntry != NULL ) {
                    ...
                    CGDone( CGAssign( dst, arg, CGTY_POINTER ) );
                    arg = CGUnary( O_POINTS,
                             CGFEName( FindArgShadow( ReturnValue ), CGTY_INTEGER ),
                             CGTY_INTEGER );
                }
                dst = SCBLenAddr( SubAltSCB( sym ) );
                CGDone( CGAssign( dst, arg, CGTY_INTEGER ) );
            }
        }
    }
    for(;;) {
        ...
        // character variable
        arg = CGUnary( O_POINTS, CGFEName( sym, CGTY_POINTER ), CGTY_POINTER );
        ...
        if( sym->ns.xt.size == 0 ) {
            ...
            if( sym->ns.flags & SY_VALUE_PARM ) {
                arg = CGInteger( 0, CGTY_INTEGER );
            } else if( Options & OPT_DESCRIPTOR ) {
                arg = CGFEName( sym, CGTY_POINTER );
                arg = CGUnary( O_POINTS, arg, CGTY_POINTER );
                arg = SCBLength( arg );
            } else {
                arg = CGFEName( FindArgShadow( sym ), CGTY_INTEGER );
                arg = CGUnary( O_POINTS, arg, CGTY_INTEGER );
            }
            CGDone( CGAssign( dst, arg, CGTY_INTEGER ) );
        }
    }
...
// Assign alternate return value to result of subroutine.
    ...
    if( EntryWithAltRets() ) {
        CGDone( CGAssign( CGFEName( ReturnValue, CGTY_INTEGER ), alt_ret,
                          CGTY_INTEGER ) );
    } else {
        CGDone( alt_ret );
    }
