...
extern  cg_name         GetAdv(sym_id);
  ...
  #define FLAG_PARM_TYPE        CGTY_UINT_2
#else
  #define FLAG_PARM_TYPE        CGTY_UINT_4
#endif
...
//======================================
    ...
    if( sym->ns.flags & SY_SUBSCRIPTED ) {
        ...
        tlen = BETypeLength( CGTY_CHAR );
        typ = CGTY_CHAR;
    }
    fl = StructRef( CGFEName( sym, typ ), tlen );
    return( CGUnary( O_POINTS, fl, CGTY_UINT_2 ) );
}
...
//============================
    ...
    for(;;) {
        ...
        label = BENewLabel();
        fl = getFlags( arr );
        fl = CGBinary( O_AND, fl, CGInteger( ALLOC_MEM, CGTY_UINT_2 ), CGTY_UINT_2 );
        CGControl( O_IF_TRUE, CGCompare( O_NE, fl,
                                CGInteger( 0, CGTY_UINT_2 ), CGTY_UINT_2 ), label );
    ...
    if( alloc_flags & ALLOC_NONE ) {
        expr_loc = CGInteger( 0, CGTY_POINTER );
    } else {
        FCodeSequence();
        if( alloc_flags & ALLOC_LOC ) {
            expr_loc = XPopValue( CGTY_INT_4 );
    ...
    for(;;) {
        ...
        if( arr->ns.flags & SY_SUBSCRIPTED ) {
            ...
            CGAddParm( handle, CGInteger( _SymSize( arr ), CGTY_INT_4 ),
                       CGTY_INT_4 );
            CGAddParm( handle, CGInteger( _DimCount( dim->dim_flags ),
                                          CGTY_INTEGER ),
                       CGTY_INTEGER );
            CGAddParm( handle, GetAdv( arr ), CGTY_LOCAL_POINTER );
        }
        CGAddParm( handle, CGFEName( arr, CGTY_POINTER ), CGTY_POINTER );
        CGAddParm( handle, getFlags( arr ), FLAG_PARM_TYPE );
    }
    if( alloc_flags & ALLOC_NONE ) {
        CGAddParm( handle, expr_loc, CGTY_POINTER );
    } else {
        if( alloc_flags & ALLOC_LOC ) {
            CGAddParm( handle, expr_loc, CGTY_INT_4 );
        }
        if( alloc_flags & ALLOC_STAT ) {
            CGAddParm( handle, expr_stat, CGTY_POINTER );
        }
    }
    CGAddParm( handle, CGInteger( num, CGTY_INTEGER ), CGTY_INTEGER );
    CGAddParm( handle, CGInteger( alloc_flags, CGTY_UINT_2 ), FLAG_PARM_TYPE );
...
//==============================
    ...
    if( GetU16() & ALLOC_STAT ) {
        ...
        CGAddParm( handle, CGInteger( 0, CGTY_POINTER ), CGTY_POINTER );
    }
    CGDone( CGCall( handle ) );
}
...
// Generate code for ALLOCATED intrinsic function.
    ...
    if( type & ALLOC_STRING ) {
        fl = CGUnary( O_POINTS, fl, CGTY_POINTER );
    }
    XPush( CGCompare( O_NE, fl, CGInteger( 0, CGTY_POINTER ), CGTY_POINTER ) );
}
