...
//==================================
    ...
    if( sym->ns.si.va.dim_ext->dim_flags & DIM_EXTENDED ) {
        ...
        if( CGOpts & CGOPT_M_LARGE ) {
            return( CGTY_HUGE_POINTER );
        } else { // if( CGOpts & CGOPT_M_MEDIUM ) {
            return( CGTY_LONG_POINTER );
        }
#elif _CPU == 386
        return( CGTY_LONG_POINTER );
#endif
...
// we don't get two run-time calls for huge pointer arithmetic.
    ...
    if( ( sym->ns.flags & SY_CLASS ) == SY_SUBPROGRAM ) {
        if( ( sym->ns.flags & SY_SUBPROG_TYPE ) == SY_STMT_FUNC ) {
            ...
            addr = CGFEName( sym, CGTY_CODE_PTR );
            if( sym->ns.flags & SY_SUB_PARM ) {
                addr = CGUnary( O_POINTS, addr, CGTY_CODE_PTR );
            }
        ...
        if( CommonEntry == NULL ) {
            if( sym->ns.typ == TY_CHAR ) {
                if( Options & OPT_DESCRIPTOR ) {
                    ...
                    addr = CGUnary( O_POINTS, addr, CGTY_POINTER );
                } else {
            ...
            if( (sym->ns.typ == TY_CHAR) && !(Options & OPT_DESCRIPTOR) ) {
                ...
                addr = CGUnary( O_POINTS, CGFEName( ReturnValue, CGTY_POINTER ),
                                CGTY_POINTER );
            }
        }
        ...
        if( i != NULL ) {
            i = CGBinary( O_PLUS, i, CGInteger( offset, CGTY_INT_4 ), CGTY_INT_4 );
        } else {
            i = CGInteger( offset, CGTY_INT_4 );
        }
        ...
        if( (sym->ns.typ == TY_CHAR) && !(sym->ns.flags & SY_SUBSCRIPTED) ) {
            ...
            addr = CGBinary( O_COMMA, addr,
                             CGFEName( sym, F772CGType( sym ) ), CGTY_DEFAULT );
        }
        i = NULL;
        ...
        ce_ext = sym->ns.si.va.vi.ec_ext;
        if( i != NULL ) {
            i = CGBinary( O_PLUS, i, CGInteger( ce_ext->offset, CGTY_INT_4 ),
                          CGTY_INT_4 );
        } else {
            i = CGInteger( ce_ext->offset, CGTY_INT_4 );
        }
...
// Process POP F-Code.
    ...
    if( ( dst_typ == CGTY_COMPLEX ) || ( dst_typ == CGTY_DCOMPLEX )
        || ( dst_typ == CGTY_XCOMPLEX ) ) {
        ...
        if( (src_typ == CGTY_COMPLEX) || (src_typ == CGTY_DCOMPLEX)
            || (src_typ == CGTY_XCOMPLEX) ) {
            Cmplx2Scalar();
...
//===================================================
    ...
    if( size == sizeof( intstar1 ) ) {
        return( CGInteger( value->intstar1, CGTY_INT_1 ) );
    } else if( size == sizeof( intstar2 ) ) {
        return( CGInteger( value->intstar2, CGTY_INT_2 ) );
    } else {
        return( CGInteger( value->intstar4, CGTY_INT_4 ) );
    }
}
...
// Process PUSH_CONST F-Code.
    ...
    case TY_LOGICAL_1 :
    case TY_LOGICAL :
        XPush( CGInteger( sym->cn.value.logstar4, CGTY_UINT_1 ) );
        break;
    case TY_REAL :
        CnvS2S( &sym->cn.value.single, fmt_buff );
        XPush( CGFloat( fmt_buff, CGTY_SINGLE ) );
        break;
    case TY_DOUBLE :
        CnvD2S( &sym->cn.value.dble, fmt_buff );
        XPush( CGFloat( fmt_buff, CGTY_DOUBLE ) );
        break;
    case TY_TRUE_EXTENDED :
        CnvX2S( &sym->cn.value.extended, fmt_buff );
        XPush( CGFloat( fmt_buff, CGTY_LONGDOUBLE ) );
        break;
    case TY_COMPLEX :
        PushCmplxConst( sym );
        break;
...
// Process PUSH_LIT F-Code.
    ...
    if( sym->lt.flags & (LT_SCB_REQUIRED | LT_SCB_TMP_REFERENCE) ) {
        XPush( CGBackName( ConstBack( sym ), CGTY_CHAR ) );
    }
}
